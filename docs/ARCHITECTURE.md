...
本文面向维护者，描述 Academic RSS Reader v1.5.0 的当前运行架构。代码、测试和
...
两个直接分区。卡片统一固定为 `13rem` 高度：内容区固定预留三行标题、一期刊行和包含
相关度/关键词/操作的底行，并使用更大的行间距；超过三行的标题通过固定高度直接裁切，
不使用 Chromium `-webkit-line-clamp`，避免在中间行错误插入省略号；悬停可查看完整文本。
操作按钮直接紧跟相关度与两行关键词区域，不使用剩余空间推到最右侧。图片区受固定
卡片高度约束，不参与决定卡片高度。图片元素自身是可点击区域，
