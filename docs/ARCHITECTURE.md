...
本文面向维护者，描述 Academic RSS Reader v1.6.3 的当前运行架构。代码、测试和
...
完整悬停文本。底部操作区使用固定的相关度列和操作列，关键词长度不会推动按钮位置。
...
阅读器列表按每批 100 条使用底部哨兵继续加载；当当前篮子的文献全部加载完成后，哨兵下方
追加返回顶部按钮，直接把阅读器滚动容器恢复到顶部。切换篮子、排序或刷新会重新计算该按钮。
...
成功或失败事件只更新目标卡片。源文本 hash 改变时旧译文不复用；请求有超时，网络、限流和
服务器错误按 15 秒、1 分钟、5 分钟退避，单任务最多尝试三次。达到上限后保留 failed 记录，
停止自动请求并由阅读器提供手动重试。目标语言由设置选择，源语言继续自动识别。
