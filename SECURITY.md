...
- 用户开启标题翻译后，可见标题会直接发送给 Google 非正式网页翻译接口；请求有 15 秒超时，网络、限流和服务器错误最多尝试三次（含首次请求），其间使用退避策略。
...
免密网页翻译属于实验性辅助阅读功能，可能限流、失效或受地区政策影响。译文不保证学术准确性，不应用于正式引用。翻译请求超时或连续失败后会停止自动重试并保留失败任务，失败不会覆盖原标题，也不会阻塞其他功能。
